...
class Ec2Inventory(object):
    ...
    def __init__(self):
        ...
        #Modified for wrapper_ec2
        #print data_to_print
        self.wrapper_ec2(data_to_print)
    ...
    #Adding below for wrapper_ec2: 
    def wrapper_ec2(self, inv):
        ''' Writes to OP_FILE, so it can be used as ansible inventory '''
        inv_dict = {}
        data = json.loads(inv)
        hosts = data["_meta"]["hostvars"]
        for k,v in hosts.iteritems():
            inv_dict[v["ec2_tag_Env"]] = inv_dict.get(v["ec2_tag_Env"],[])+[k]
        with open(OP_FILE,"w+") as op: 
            for env, ip_list in inv_dict.iteritems():
                op.write('[{0}]\n{1}\n\n'.format(env, "\n".join(ip_list)))
